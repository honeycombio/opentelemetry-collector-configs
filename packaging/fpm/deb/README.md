# Build otel-hny-collector deb package

Build the otel-hny-collector deb package with [fpm](https://github.com/jordansissel/fpm).

To build the deb package, run `make deb-package` from the repo root directory. The deb package will be written to
`dist/otel-hny-collector_<version>_<arch>.deb`.
